...
a2_biased = [ones(size(a2, 1), 1) a2];

z3 = a2_biased * Theta2';

a3 = sigmoid(z3);

unregularized_J = sum(sum((-Y .* log(a3)) - ((1 - Y) .* log(1 - a3)))) / m;

Theta1_reduced = Theta1;
Theta1_reduced(:,[1]) = [];
Theta2_reduced = Theta2;
Theta2_reduced(:,[1]) = [];

J = unregularized_J + (sum(sum(Theta1_reduced .^ 2)) + sum(sum(Theta2_reduced .^ 2))) * lambda / (2 * m);
